...
def train(
    ...
    spk_encoder = None
    if spk_loss and spk_loss.get("enable", False):
        checkpoint = spk_loss.get("checkpoint", "")
        if not checkpoint:
            raise ValueError("spk_loss.checkpoint is required when spk_loss.enable=True")
        spk_encoder = load_latent_speaker_encoder(checkpoint, accelerator.device)
        if accelerator.rank == 0:
            tracker.print(f"Loaded latent speaker encoder for loss/spk from {checkpoint}")
        ...
        for step in range(start_step, num_iters):
            ...
            if val_loader is not None and (step % valid_interval == 0 or step == num_iters - 1):
                validate(
                    ...
                    spk_encoder=spk_encoder if spk_loss and spk_loss.get("val_enable", False) else None,
                    spk_loss_config=spk_loss,
                )
...
def load_latent_speaker_encoder(checkpoint: str, device: torch.device):
    """Load the frozen Latent_embedding speaker encoder from the workspace."""
    checkpoint_path = Path(checkpoint)
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"spk_loss.checkpoint not found: {checkpoint}")

    candidates = [
        project_root.parent / "Latent_embedding" / "models.py",
        project_root / "Latent_embedding" / "models.py",
    ]
    module_path = next((path for path in candidates if path.exists()), None)
    if module_path is None:
        raise FileNotFoundError(
            "Could not find Latent_embedding/models.py. "
            f"Checked: {', '.join(str(path) for path in candidates)}"
        )

    spec = importlib.util.spec_from_file_location("latent_embedding_models_for_voxcpm", module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not import latent speaker model from {module_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)

    model = module.LatentSpeakerEncoder.from_checkpoint(checkpoint_path, map_location=device)
