...
class VoxCPM2Model(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        spk_encoder: nn.Module | None = None,
        spk_loss_config: dict | None = None,
    ):
        ...
        loss_spk = self._compute_spk_loss(
            ...
            spk_loss_config=spk_loss_config,
        )
        if loss_spk is not None:
            outputs["loss/spk"] = loss_spk
        return outputs
    ...
    @staticmethod
    def _even_chunk_starts(total_len: int, chunk_size: int, count: int) -> list[int]:
        max_start = total_len - chunk_size
        if count <= 1 or max_start <= 0:
            return [0]
        starts = torch.linspace(0, max_start, steps=count).round().to(torch.long).tolist()
        return sorted(set(int(s) for s in starts))

    def _check_spk_encoder_compat(self, spk_encoder: nn.Module):
        enc_cfg = getattr(spk_encoder, "config", None)
        if enc_cfg is None:
            return
        if int(getattr(enc_cfg, "patch_size", self.patch_size)) != int(self.patch_size):
            raise ValueError(
                f"spk_encoder patch_size={getattr(enc_cfg, 'patch_size')} "
                f"does not match VoxCPM patch_size={self.patch_size}"
            )
        if int(getattr(enc_cfg, "feat_dim", self.feat_dim)) != int(self.feat_dim):
            raise ValueError(
                f"spk_encoder feat_dim={getattr(enc_cfg, 'feat_dim')} "
                f"does not match VoxCPM feat_dim={self.feat_dim}"
            )

    def _spk_rollout_dit_hidden(
        self,
        text_tokens: torch.Tensor,
        text_mask: torch.Tensor,
        audio_feats: torch.Tensor,
        ...
        return torch.cat((self.lm_to_dit_proj(lm_hidden), self.res_to_dit_proj(residual_hidden)), dim=-1)

    def _spk_build_prefix_feats(
        self,
    ...
    def _rollout_spk_chunk(
        ...
        *,
        ...
        rollout_audio_start: int,
        chunk_audio_start: int,
        chunk_size: int,
        n_timesteps: int,
        cfg_value: float,
        temperature: float,
        ...
        for audio_idx in range(rollout_audio_start, rollout_audio_end):
            ...
            dit_hidden = self._spk_rollout_dit_hidden(
                text_tokens=prefix_text_tokens,
                text_mask=prefix_text_mask,
            ...
            pred = self.feat_decoder.sample(
                mu=dit_hidden,
                patch_size=self.patch_size,
                cond=cond_patch.unsqueeze(0).transpose(1, 2).contiguous(),
                n_timesteps=n_timesteps,
                cfg_value=cfg_value,
                temperature=temperature,
            )
            generated_by_pos[abs_pos] = pred.transpose(1, 2).squeeze(0).to(audio_feats.dtype)

        chunk_positions = [int(pos.item()) for pos in target_indices[chunk_audio_start : chunk_audio_start + chunk_size]]
        return torch.stack([generated_by_pos[pos] for pos in chunk_positions], dim=0)

    def _compute_spk_loss(
        self,
        *,
        text_tokens: torch.Tensor,
        text_mask: torch.Tensor,
        ...
        audio_mask: torch.Tensor,
        loss_mask: torch.Tensor,
        spk_encoder: nn.Module | None,
        spk_loss_config: dict | None,
    ) -> torch.Tensor | None:
        if spk_encoder is None:
            return None
        cfg = spk_loss_config or {}
        if not cfg.get("enable", True):
            return None

        self._check_spk_encoder_compat(spk_encoder)
        chunk_size = int(cfg.get("chunk_size", 50))
        num_gen_chunks = max(1, int(cfg.get("num_gen_chunks", 2)))
        num_ref_chunks = max(1, int(cfg.get("num_ref_chunks", 4)))
        min_progress = float(cfg.get("min_progress", 0.2))
        rollout_from_progress = float(cfg.get("rollout_from_progress", min_progress))
        progress_power = float(cfg.get("progress_power", 2.0))
        ...
        for b in range(audio_feats.size(0)):
            target_indices = (loss_mask[b] > 0.5).nonzero(as_tuple=True)[0]
            total_audio_len = int(target_indices.numel())
            if total_audio_len < chunk_size:
                continue

            max_start = total_audio_len - chunk_size
            ref_starts = self._even_chunk_starts(total_audio_len, chunk_size, num_ref_chunks)
            gt_chunks = torch.stack(
                [audio_feats[b, target_indices[start : start + chunk_size], :, :] for start in ref_starts],
                dim=0,
            )
            lengths = lengths_cache.get(gt_chunks.size(0))
            if lengths is None:
                lengths = torch.full(
                    (gt_chunks.size(0),),
                    chunk_size,
                ...
                lengths_cache[gt_chunks.size(0)] = lengths
            gt_emb = spk_encoder(gt_chunks.float(), lengths)
            target_emb = F.normalize(gt_emb.mean(dim=0, keepdim=True), dim=-1).detach()

            late_min_start = min(max_start, max(0, int(total_audio_len * min_progress)))
            candidates = torch.arange(late_min_start, max_start + 1, device=audio_feats.device)
            if candidates.numel() == 0:
                continue
            candidate_progress = (candidates.float() + chunk_size) / max(1.0, float(total_audio_len))
            weights = torch.clamp(candidate_progress, min=1e-3).pow(progress_power)
            sampled = candidates[torch.multinomial(weights, num_samples=num_gen_chunks, replacement=True)]

            gen_chunks = []
            for chunk_audio_start in sampled.tolist():
                rollout_audio_start = min(
                    int(chunk_audio_start),
                    max(0, int(total_audio_len * rollout_from_progress)),
                )
                gen_chunk = self._rollout_spk_chunk(
                    text_tokens=text_tokens[b],
                    text_mask=text_mask[b],
                    audio_feats=audio_feats[b],
                    audio_mask=audio_mask[b],
                    target_indices=target_indices,
                    rollout_audio_start=rollout_audio_start,
                    chunk_audio_start=int(chunk_audio_start),
            ...
            gen_chunks = torch.stack(gen_chunks, dim=0)
            gen_lengths = lengths_cache.get(gen_chunks.size(0))
            if gen_lengths is None:
                ...
                lengths_cache[gen_chunks.size(0)] = gen_lengths
            gen_emb = spk_encoder(gen_chunks.float(), gen_lengths)
            losses.append(1.0 - F.cosine_similarity(gen_emb, target_emb.expand_as(gen_emb), dim=-1).mean())
